...
└────────────┬─────────────┘
             ├──► 二次检索（reporter 可单独补文本/数据，回 retriever → reporter）
             ▼
...
> `data_tool` 节点会自动注入 `session_id`、`settings`。
...
```json
{
  "action": "call_tool|retrieve_text|retrieve_data|done",
...
**测试目的**：验证入口校验、enable_* 开关、data_process_plan、会话历史数据目录等 planner 规则是否符合预期。
...
**测试目的**：验证 `data_process_plan` + 工具历史 + `params.artifact_description` 下的 call_tool / done / replan。

#### reporter

```bash
poetry run python -m scripts.prompt_debug.run_reporter --query "汇总龙虎榜"
poetry run python -m scripts.prompt_debug.run_reporter --state scripts/prompt_debug/samples/reporter_default.json
...
flowchart TD
    ...
    reporter -->|retrieve_text / retrieve_data| retriever
